...
import type { Filters } from '@/api/admin/packs/getEggs';
import getEggs, { Context as EggsContext } from '@/api/admin/packs/getEggs';
import AdminTable, {
    TableBody,
    TableHead,
    TableHeader,
...
const EggsTable = () => {
    const params = useParams<'packId' | 'id'>();

    const { setPage, setFilters, sort, setSort, sortDirection } = useContext(EggsContext);
    const { clearFlashes, clearAndAddHttpError } = useFlash();
    const { colors } = useStoreState(state => state.theme.data!);
    const { data: eggs, error, isValidating } = getEggs(Number(params.packId));
                                ...
                                {eggs !== undefined &&
                                    ...
                                    eggs.items.map(egg => (
                                        <TableRow key={egg.id}>
                                            ...
                                            <td css={tw`px-6 text-sm text-neutral-200 text-left whitespace-nowrap`}>
                                                <NavLink
                                                    to={`/admin/packs/${params.packId}/eggs/${egg.id}`}
