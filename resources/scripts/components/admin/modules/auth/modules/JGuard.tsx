...
export default () => {
    ...
    return (
        <AdminBox title={'jGuard'} icon={faDoorOpen}>
            ...
            />
            <div>
                <Label>{t('auth.jguardDelay', 'Automatic approval delay')}</Label>
                <Input
                    id={'delay'}
                    type={'text'}
                    name={'delay'}
                ...
                <p className={'text-xs text-gray-400 mt-1'}>
                    {t('auth.jguardDelayDesc', 'If you wish to automatically approve user signups, this variable can make it so that users cannot access the Panel for a certain period of time in order to prevent bot attacks.')}
                </p>
            </div>
        </AdminBox>
    );
