...
export default () => {
    ...
    return (
        <AdminContentBlock showFlashKey={'theme:colors'}>
            <Dialog.Confirm
                title={t('theme.confirmReset', 'Are you sure?') as string}
                open={visible}
                onClose={() => setVisible(false)}
                onConfirmed={submit}
            >
                {t('theme.resetWarning', 'Performing this action will immediately wipe all of your custom theming settings. Only do this if you wish to return to the stock appearance of Jexactyl. This action cannot be reversed.')}
            </Dialog.Confirm>
            <div className={'w-full flex flex-row items-center mb-8'}>
                <div className={'flex flex-col flex-shrink'} style={{ minWidth: '0' }}>
                    <h2 className={'text-2xl text-neutral-50 font-header font-medium'}>{t('theme.title', 'System Theme')}</h2>
                    <p
                        className={
                            'hidden lg:block text-neutral-400 whitespace-nowrap overflow-ellipsis overflow-hidden'
                        }
                    >
                        {t('theme.description', 'View and update the theme of this interface.')}
                    </p>
                </div>
                <div className={'flex ml-auto pl-4'}>
                    ...
                    >
                        {t('theme.resetToDefaults', 'Reset to Defaults')}
                    </Button>
                </div>
            </div>
