    ...
    return (
        <Formik
            ...
            validationSchema={object().shape({
                email: string()
                    .max(191, t('users.validation.emailMax', 'Email addresses must not exceed 191 characters.') as string)
                    .email(t('users.validation.emailValid', 'A valid email address must be provided.') as string)
                    .required(t('users.validation.emailRequired', 'A valid email address must be provided.') as string),
            ...
            <Form>
                <div css={tw`flex justify-between`}>
                    <h2 css={tw`text-2xl`} ref={ref}>
                        {subuser
                            ? `${t('users.modifyPermissions', 'Modify permissions for')} ${subuser.email}`
                            : t('users.createDescription', 'Create new subuser')}
                    </h2>
                    <div>
                        <Button type={'submit'} css={tw`w-full sm:w-auto`}>
                            {subuser ? t('users.save', 'Save') : t('users.invite', 'Invite User')}
                        </Button>
                    </div>
                </div>
                ...
                {!isRootAdmin && loggedInPermissions[0] !== '*' && (
                    <div css={tw`mt-4 pl-4 py-2 border-l-4 border-cyan-400`}>
                        <p css={tw`text-sm text-neutral-300`}>
                            {t('users.onlyPermissionsWarning', 'Only permissions which your account is currently assigned may be selected when creating or modifying other users.')}
                        </p>
                    </div>
                )}
                {!subuser && (
                    <div css={tw`mt-6`}>
                        <Field
                            name={'email'}
                            label={t('users.email', 'User Email') as string}
                            description={t('users.emailDescription', 'Enter the email address of the user you wish to invite as a subuser for this server.') as string}
                        />
                    </div>
                )}
                <div css={tw`my-6`}>
