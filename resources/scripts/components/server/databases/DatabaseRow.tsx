    ...
    return (
        <>
            <Formik onSubmit={submit} initialValues={{ confirm: '' }} validationSchema={schema} isInitialValid={false}>
                {({ isSubmitting, isValid, resetForm }) => (
                    ...
                    >
                        ...
                        <h2 css={tw`text-2xl mb-6`}>{t('databases.confirmDelete')}</h2>
                        <p css={tw`text-sm`}>
                            {t('databases.deleteWarning', { name: database.name })}
                        </p>
                        <Form css={tw`m-0 mt-6`}>
                            <Field
                                ...
                                id={'confirm_name'}
                                name={'confirm'}
                                label={t('databases.confirmName') as string}
                                description={t('databases.confirmNameDescription') as string}
                            />
            ...
            <Modal visible={connectionVisible} onDismissed={() => setConnectionVisible(false)}>
                ...
                <h3 css={tw`mb-6 text-2xl`}>{t('databases.connectionDetails')}</h3>
                <div>
                    <Label>{t('databases.endpoint')}</Label>
                    <CopyOnClick text={database.connectionString}>
                        <Input type={'text'} readOnly value={database.connectionString} />
                    </CopyOnClick>
                </div>
                <div css={tw`mt-6`}>
                    <Label>{t('databases.connectionsFrom')}</Label>
                    <Input type={'text'} readOnly value={database.allowConnectionsFrom} />
                </div>
                <div css={tw`mt-6`}>
                    <Label>{t('databases.username')}</Label>
                ...
                <Can action={'database.view_password'}>
                    <div css={tw`mt-6`}>
                        <Label>{t('databases.password')}</Label>
                ...
                <div css={tw`mt-6`}>
                    <Label>{t('databases.jdbcString')}</Label>
                ...
                <div css={tw`mt-6 text-right`}>
                    ...
                    <Button onClick={() => setConnectionVisible(false)}>{t('databases.close')}</Button>
                </div>
            </Modal>
            <GreyRowBox $hoverable={false} className={className} css={tw`mb-2`}>
                ...
                <div css={tw`ml-8 text-center hidden md:block`}>
                    ...
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>{t('databases.endpoint')}</p>
                </div>
                <div css={tw`ml-8 text-center hidden md:block`}>
                    <p css={tw`text-sm`}>{database.allowConnectionsFrom}</p>
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>{t('databases.connectionsFrom')}</p>
                </div>
                <div css={tw`ml-8 text-center hidden md:block`}>
                    <CopyOnClick text={database.username}>
                        <p css={tw`text-sm`}>{database.username}</p>
                    </CopyOnClick>
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>{t('databases.username')}</p>
                </div>
                <div css={tw`ml-8`}>
