...
export const rawDataToPack = ({ attributes }: FractalResponseData): Pack => ({
    id: attributes.id,
...
export default (include: string[] = []) => {
    ...
    return useSWR<PaginatedResult<Pack>>(['packs', page, filters, sort, sortDirection], async () => {
        const { data } = await http.get('/api/application/packs', {
            params: { include: include.join(','), page, ...params },
        });

        return {
            items: (data.data || []).map(rawDataToPack),
